...
local function setSelectedMesh(currentMeshName, meshID, noCallbacks)
	--cada combo tem a sua propria lista: a do corpo e montada com show_children e a
	--da animacao normalmente sem, entao os indices nao batem. Procurar sempre em
	--meshList fazia o combo da animacao cair no item errado e zerar o parametro.
	local list = (meshID == "animation_mesh_combo") and animationMeshList or meshList
	local selectedIndex = 1
	for i = 1, #list do
		if list[i] == currentMeshName then
			selectedIndex = i
			break
		end
	end
