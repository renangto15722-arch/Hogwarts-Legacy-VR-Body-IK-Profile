	...
	if fileName ~= nil and fileName ~= "" and json ~= nil then
		...
		if loadConfig ~= nil then
			for key, val in pairs(loadConfig) do
				...
				-- Um arquivo NÃO ROUBA um id que já pertence a outro painel
				-- (14/08/2026). M.create termina carregando o config_default, e o
				-- laço abaixo reapontava itemMap[key] para ele — bastava uma chave
				-- velha sobrar naquele arquivo para o getValue/setValue do id
				-- passarem a ler e escrever LÁ, enquanto o widget do painel de
				-- verdade continuava mostrando e salvando o valor certo no arquivo
				-- dele. Sintoma: o campo mexe, salva, e nada acontece no jogo,
				-- porque quem lê recebe para sempre o valor congelado do outro
				-- arquivo. Aconteceu com o "Altura dos olhos" do Corpo VR.
				-- O caminho legítimo continua valendo: chave sem dono nenhum é
				-- mapeada aqui (é assim que item dinâmico, criado fora de painel,
				-- é encontrado), e um painel criado depois assume o id na
				-- createPanel, como sempre.
				...
				if itemMap[key] == nil or itemMap[key] == panelID then
					itemMap[key] = panelID
				end
